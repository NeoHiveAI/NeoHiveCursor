...
Now that the MCP is reachable, generate a project-specific topology block in `./AGENTS.md`. This is what makes the model reliable about _which_ hive to query and _where_ new writes should land — without it, the always-applied rule in `rules/neohive.mdc` is running blind.
