...
Before splitting a project file into candidate memories, strip any content between `<!-- BEGIN neohive-managed v=N -->` and `<!-- END neohive-managed v=N -->` markers. The marker block is generated by the `generate-agents-md` skill and is intentionally not migrated — it's the topology/routing reference the model needs in context BEFORE `memory_context` is called, so it must stay in AGENTS.md.
...
Break each project-scoped file (with marker blocks already stripped) into **atomic candidate memories**. A candidate is a single self-contained directive, convention, decision, or insight that would make sense read in isolation 6 months from now.
