...
Rule of thumb: _if the memory system already had this insight, the assistant should have found it during the session. If it didn't, either the insight is new, or the existing memory's embedding doesn't cover this query angle — either way, storing a new entry is correct._
